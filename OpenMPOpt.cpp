...
/* */
static cl::opt<bool>
    KernelMemoryOptimize("kernel-memory-optimize", cl::ZeroOrMore,
                         cl::desc("Enables kernel memory optimization."),
                         cl::Hidden, cl::init(false));
static cl::opt<bool> StaticKernelMemoryOptimize(
    "static-kernel-memory-optimize", cl::ZeroOrMore,
    cl::desc("Enables static kernel memory optimization."), cl::Hidden,
    cl::init(false));

static cl::opt<std::string> KernelMemoryOptimizeMode(
    "kernel-memory-optimize-mode", cl::ZeroOrMore,
    cl::desc("Specifies the mode of kernel memory optimization."), cl::Hidden,
    cl::init("different"));
...
private:
  ...
  bool mergeParallelRegions() {
    ...
    if (Changed) {
      ...
      // OMPInfoCache.recollectUsesForFunction(OMPRTL_copy_to_shared_mem);
  ...
  bool runAttributor(bool IsModulePass) {
    ...
    ExternalizationRAII SharedMem(OMPInfoCache, OMPRTL_copy_to_shared_mem);
...
struct AAKernelInfoCallSite : AAKernelInfo {
  ...
  void initialize(Attributor &A) override {
    ...
    case OMPRTL___kmpc_nvptx_end_reduce_nowait:
      // case OMPRTL_copy_to_shared_mem:
      break;
...
struct AAFoldRuntimeCallCallSiteReturned : AAFoldRuntimeCall {
  ...
  ChangeStatus updateImpl(Attributor &A) override {
    ...
    case OMPRTL___kmpc_get_hardware_num_blocks:
      ...
      /**/
      // case OMPRTL_copy_to_shared_mem:
      //   Changed = Changed | foldKernelFnAttribute(A, "copy_to_shared_mem");
      //   break;
      // default:
...
void OpenMPOpt::registerAAs(bool IsModulePass) {
  ...
  if (IsModulePass) {
    ...
    registerFoldRuntimeCall(OMPRTL_copy_to_shared_mem);
  }
...
// TODO: FIX these two functions and replace with more efficient, better ones
// !!!
//  It's a bad method to look for calls with their name !!!
//  Also, these are not generalized, but they can be, so do that as well
Function *funcByName(Module &M, std::string Name) {
  for (auto &f : M)
    if (f.getName() == Name)
      return &f;
  return NULL;
...
CallInst *callByName(Function *F, std::string Name) {
  CallInst *Ci;
  for (auto &I : instructions(*F)) {
...
// the case that every iteration has its own, different from others base start.
int kernelMemOpt(Function *Kernel, FunctionAnalysisManager &FAM,
                 Attributor &A) {
  Value *const1 =
      ConstantInt::get(IntegerType::get(Kernel->getContext(), 32), 1);
  Value *const164 =
      ConstantInt::get(IntegerType::get(Kernel->getContext(), 64), 1);
  Value *const0 =
      ConstantInt::get(IntegerType::get(Kernel->getContext(), 32), 0);
  Value *const064 =
      ConstantInt::get(IntegerType::get(Kernel->getContext(), 64), 0);

  Type *IntTy32 = IntegerType::get(Kernel->getContext(), 32);
  Type *IntTy64 = IntegerType::get(Kernel->getContext(), 64);
  Type *Int32PointerTy = PointerType::get(IntTy32, 0);
  Type *VoidTy = Type::getVoidTy(Kernel->getContext());

  Module &M = *Kernel->getParent();
  if (StaticKernelMemoryOptimize) {
    Function *FTeamStatic = funcByName(M, "__omp_outlined__");
    assert(FTeamStatic && "Cannot find function!");

    CallInst *CIDistStatic =
        callByName(FTeamStatic, "__kmpc_distribute_static_init_4");
    assert(CIDistStatic && "Cannot find function call!");

    BasicBlock *BTeamStatic = CIDistStatic->getParent();
    BasicBlock *TeamWorkStatic =
        BTeamStatic->getTerminator()->getSuccessor(0); // is it always the case?
    BasicBlock *SharedEndStatic = SplitEdge(BTeamStatic, TeamWorkStatic);
    SharedEndStatic->setName("shared.mem.end");
    BasicBlock *SharedPreMemStatic = SplitEdge(BTeamStatic, SharedEndStatic);
    SharedPreMemStatic->setName("shared.pre.mem");
    BasicBlock *SharedMemStatic =
        SplitEdge(SharedPreMemStatic, SharedEndStatic);
    SharedMemStatic->setName("shared.mem");
    return 1;
  }
  Function *F = funcByName(M, "__omp_outlined__1");
  assert(F && "Cannot find function!");

  Function *FTeam = funcByName(M, "__omp_outlined__");
  assert(FTeam && "Cannot find function!");

  SmallVector<int> ArgNum;
  for (unsigned argnum = 0; argnum < F->arg_size(); ++argnum) {
    Value *v = F->getArg(argnum);
    for (User *U : v->users())
      if (auto g = dyn_cast<GetElementPtrInst>(U)) // it's an array
        for (User *U1 : g->users()) {
          if (dyn_cast<LoadInst>(U1)) // it's a read access
            ArgNum.push_back(argnum);
          else
            break;
        }
  }
  Value *v1 = F->getArg(ArgNum[0]); // v1
  GetElementPtrInst *V1Gep;
  SmallVector<GetElementPtrInst *> AllV1Gep;
  for (User *U : v1->users())
    if (auto g = dyn_cast<GetElementPtrInst>(U))
      AllV1Gep.push_back(g);

  V1Gep = AllV1Gep[0];
  Value *NTeam = FTeam->getArg(2);
  Value *NThread = F->getArg(4);
  // it has just one index, for linearized arrays, it is
  // always the case.
  Value *idx = *(V1Gep->idx_begin());
  ScalarEvolution &SE = FAM.getResult<ScalarEvolutionAnalysis>(*F);
  const DataLayout &DLModule = M.getDataLayout();
  SCEVExpander *SCEVExpr = new SCEVExpander(SE, DLModule, "expander");
  auto ISCEV = SE.getSCEV(cast<Value>(idx));
  const SCEVAddRecExpr *addRec = dyn_cast<SCEVAddRecExpr>(ISCEV);
  const Loop *addRecLoop = addRec->getLoop();
  PHINode *AccessIV = addRecLoop->getCanonicalInductionVariable();
  CmpInst *LoopCmp = cast<CmpInst>(
      cast<BranchInst>(AccessIV->getParent()->getTerminator())->getCondition());
  Value *Num1 = LoopCmp->getOperand(1);

  // put it where we have everything
  const SCEV *Start = addRec->getStart();
  SCEVExpr->setInsertPoint(V1Gep);
  Value *StartVal = SCEVExpr->expandCodeFor(Start);
  Instruction *StartClone = cast<Instruction>(StartVal)->clone();
  SmallVector<Value *> OtherGepOff;
  for (unsigned i = 1; i < AllV1Gep.size(); ++i) {
    Value *idx1 = *(AllV1Gep[i]->idx_begin());
    auto ISCEV1 = SE.getSCEV(cast<Value>(idx1));
    const SCEVAddRecExpr *addRec1 = dyn_cast<SCEVAddRecExpr>(ISCEV1);
    const SCEV *Start1 = addRec1->getStart();
    SCEVExpr->setInsertPoint(AllV1Gep[i]);
    Value *StartVal1 = SCEVExpr->expandCodeFor(Start1);
    BinaryOperator *OtherOff = BinaryOperator::CreateSub(
        StartVal1, StartVal, "other.offset", AllV1Gep[i]);
    OtherGepOff.push_back(OtherOff);
  }

  CallInst *CIStaticInit = callByName(F, "__kmpc_for_static_init_8u");
  assert(CIStaticInit && "Cannot find function call!");
  ;

  BasicBlock *B = CIStaticInit->getParent();
  CallInst *CIDist = callByName(FTeam, "__kmpc_distribute_static_init_8u");
  assert(CIDist && "Cannot find function call!");
  ;

  BasicBlock *BTeam = CIDist->getParent();

  BasicBlock *OmpLoop = B->getTerminator()->getSuccessor(0);
  BasicBlock *TeamWork =
      BTeam->getTerminator()->getSuccessor(0); // is it always the case?
  BasicBlock *SharedEnd = SplitEdge(BTeam, TeamWork);
  SharedEnd->setName("shared.mem.end");
  BasicBlock *SharedPreMem = SplitEdge(BTeam, SharedEnd);
  SharedPreMem->setName("shared.pre.mem");
  Instruction *SharedPreMemInsertPoint = SharedPreMem->getTerminator();

  BasicBlock *SharedMem = SplitEdge(SharedPreMem, SharedEnd);
  SharedMem->setName("shared.mem");
  Instruction *SharedMemInsertPoint = SharedMem->getTerminator();
  /// multiple access to the pre-fetched line changes the Num value
  Value *MaxNumPlus =
      new AllocaInst(IntTy64, 0, 0, "max.add.to.num", SharedPreMemInsertPoint);
  new StoreInst(const064, MaxNumPlus, SharedPreMemInsertPoint);
  // it is not general, just working
  Value *TeamLB = cast<PHINode>(TeamWork->front()).getIncomingValue(0);
  Value *TeamUB = cast<Value>(CIDist->getNextNonDebugInstruction()
                                  ->getNextNonDebugInstruction()
                                  ->getNextNonDebugInstruction());
  Instruction *OmpIv = &(OmpLoop->front());

  SmallVector<Value *> InstCloneR;
  SmallVector<Value *> InstClone;
  Value *cur = StartClone;
  unsigned i = 0;
  InstCloneR.push_back(cur);
  while (1) {
    if (i >= InstCloneR.size())
      break;
    cur = InstCloneR[i];
    i++;
    Instruction *curInst = cast<Instruction>(cur);
    for (unsigned op = 0; op < curInst->getNumOperands(); ++op)
      if (dyn_cast<Instruction>(curInst->getOperand(op)) &&
          curInst->getOperand(op) != OmpIv)
        InstCloneR.push_back(curInst->getOperand(op));
  }

  for (int k = InstCloneR.size() - 1; k >= 1; --k)
    InstClone.push_back(InstCloneR[k]);
  SmallVector<Instruction *> ToMap;
  PHINode *SharedIV =
      PHINode::Create(IntTy64, 2, "shared.iv", SharedMemInsertPoint);
  for (int k = InstClone.size() - 1; k >= 1; --k) {
    Instruction *OpClone = cast<Instruction>(InstClone[k])->clone();
    OpClone->insertBefore(SharedMemInsertPoint);
    ToMap.push_back(OpClone);
  }

  for (auto Ic : InstClone) {
    Instruction *OpClone = cast<Instruction>(Ic)->clone();
    OpClone->insertBefore(SharedMemInsertPoint);
    ToMap.push_back(OpClone);
  }

  ToMap.push_back(StartClone);
  StartClone->insertBefore(SharedMemInsertPoint);

  for (auto U : OmpIv->users())
    if (dyn_cast<Instruction>(U)->getParent() == SharedMem)
      U->replaceUsesOfWith(OmpIv, SharedIV);

  for (unsigned k = 0; k < InstClone.size(); ++k)
    for (auto U : InstClone[k]->users())
      if (dyn_cast<Instruction>(U)->getParent() == SharedMem)
        U->replaceUsesOfWith(InstClone[k], ToMap[k]);

  for (auto U : NThread->users())
    if (dyn_cast<Instruction>(U)->getParent() == SharedMem)
      U->replaceUsesOfWith(NThread, NTeam);
  BinaryOperator *IVNext = BinaryOperator::CreateAdd(
      const164, SharedIV, "shared.iv.next", SharedMemInsertPoint);

  SharedIV->addIncoming(TeamLB, SharedPreMem);
  SharedIV->addIncoming(IVNext, SharedMem);

  Instruction *IVCheck;
  if (KernelMemoryOptimizeMode != "same")
    IVCheck = ICmpInst::Create(ICmpInst::ICmp, llvm::CmpInst::ICMP_SLE, IVNext,
                               TeamUB, "iv.check", SharedMemInsertPoint);
  else // always false to just do the loop once
    IVCheck = ICmpInst::Create(ICmpInst::ICmp, llvm::CmpInst::ICMP_SLE, const1,
                               const0, "iv.check", SharedMemInsertPoint);

  SharedMem->getTerminator()->eraseFromParent();
  BranchInst::Create(SharedMem, SharedEnd, IVCheck, SharedMem);

  BasicBlock *GetSharedBlock = SplitEdge(B, OmpLoop);
  GetSharedBlock->setName("shared.mem.get");
  Instruction *GetSharedInsertPoint = GetSharedBlock->getTerminator();

  GlobalValue *BaseLoc0 = M.getNamedValue("BaseLoc");

  ArrayRef<Type *> GetSharedArgsTy = {VoidTy};
  FunctionType *GetSharedFnTy =
      FunctionType::get(Int32PointerTy, GetSharedArgsTy, 0);
  Function *GetSharedFn = M.getFunction("shared_copy_thread");
  Value *GetShared = CallInst::Create(GetSharedFnTy, GetSharedFn, {},
                                      "shared.mem", GetSharedInsertPoint);
  // for padding
  ArrayRef<Type *> PaddingArgsTy = {IntTy64};
  FunctionType *PaddingFnTy = FunctionType::get(IntTy64, PaddingArgsTy, 0);
  Function *PaddingFn = M.getFunction("padding_func");
  Value *Num = CallInst::Create(PaddingFnTy, PaddingFn, {Num1}, "padded.num",
                                GetSharedInsertPoint);

  if (KernelMemoryOptimizeMode == "different") {
    // for all different bases
    Value *ThreadTeamLB = F->getArg(2);
    BinaryOperator *TempOff =
        BinaryOperator::CreateSub(OmpIv, ThreadTeamLB, "temp.off", V1Gep);
    BinaryOperator *TempOff1 =
        BinaryOperator::CreateMul(TempOff, Num, "temp.off", V1Gep);

    BinaryOperator *BufOff =
        BinaryOperator::CreateAdd(TempOff1, AccessIV, "buf.off", V1Gep);

    GetElementPtrInst *SharedV1Gep = GetElementPtrInst::CreateInBounds(
        IntTy32, GetShared, {BufOff}, "shared.arr.idx", V1Gep);
    V1Gep->replaceAllUsesWith(SharedV1Gep);
  }

  if (KernelMemoryOptimizeMode == "same") {
    GetElementPtrInst *SharedV1Gep = GetElementPtrInst::CreateInBounds(
        IntTy32, GetShared, {AccessIV}, "shared.arr.idx", V1Gep);
    V1Gep->replaceAllUsesWith(SharedV1Gep);
  }

  if (KernelMemoryOptimizeMode == "mix") {
    Value *ThreadTeamLB = F->getArg(2);
    Value *BaseLoc =
        new AddrSpaceCastInst(BaseLoc0,
                              PointerType::getWithSamePointeeType(
                                  cast<PointerType>(BaseLoc0->getType()),
                                  (unsigned int)AddressSpace::Generic),
                              BaseLoc0->getName() + ".generic", V1Gep);
    BinaryOperator *TempOff =
        BinaryOperator::CreateSub(OmpIv, ThreadTeamLB, "temp.off", V1Gep);
    GetElementPtrInst *BaseLocGep = GetElementPtrInst::CreateInBounds(
        IntTy32, BaseLoc, {TempOff}, "base.loc.idx", V1Gep);

    LoadInst *BaseLocOff = new LoadInst(IntTy32, BaseLocGep, "base.loc", V1Gep);
    BinaryOperator *TempOff1 =
        BinaryOperator::CreateMul(BaseLocOff, Num, "temp.off", V1Gep);
    BinaryOperator *BufOff =
        BinaryOperator::CreateAdd(TempOff1, AccessIV, "buf.off", V1Gep);
    GetElementPtrInst *SharedV1Gep = GetElementPtrInst::CreateInBounds(
        IntTy32, GetShared, {BufOff}, "shared.arr.idx", V1Gep);
...
PreservedAnalyses OpenMPOptPass::run(Module &M, ModuleAnalysisManager &AM) {
  ...
  /* */
  if (KernelMemoryOptimize) {
    KernelMemoryOptimize = 0;
    for (auto kernel : Kernels) {
      kernelMemOpt(kernel, FAM, A);
    }
    // errs() << "Module after OpenMPOpt CGSCC Pass:\n" << M <<
    // "********************************\n";
...
PreservedAnalyses OpenMPOptCGSCCPass::run(LazyCallGraph::SCC &C,
  ...
  //   for(auto kernel: Kernels)
  //   {
  //     kernelMemOpt(kernel, FAM.getResult<ScalarEvolutionAnalysis>(*kernel),
  //                          FAM.getResult<LoopAnalysis>(*kernel),
  //                          A);
  //   }
  //   // errs() << "Module after OpenMPOpt CGSCC Pass:\n" << M <<
  //   "********************************\n"; return PreservedAnalyses::none();
